<%@ Page Language="C#" AutoEventWireup="true" CodeFile="YqxkjMemberCoursePlay.aspx.cs" Inherits="NGWeiXinRoot_YqxkjMemberCoursePlay" %>
...
<%--		<ul class="course-list left course-info-top">
		...
		<div class="course-info">
			<p class="evaluate-title left">评价（<asp:Literal runat="server" ID="ltCommnetCount"></asp:Literal>条）<a id="makeCommentBtn"><i class="icon iconfont icon-comment"></i>评论</a></p>
		...
		<div class="cover" id="makeComment">
			<div class="model">
				...
				<p class="write-comment">
					<b>评论：</b>
					<textarea id="txt_msg" style="width:calc(100% - 6em)">挺好的</textarea>
				</p>
				<p class="btn-bar">
...
<script language="javascript" type="text/javascript">
    function to_question() {
        ...
        if ($("#hidMorePropertys").val() == "模拟试题") {
            location.href = 'YqxkjQuestionMN.aspx?memberId=' + mid + '&fromfree=nofree&source=mnlx&courseId=' + $("#hid_goodsId").val() + '&r=' + Math.random();
        } else {
            location.href = 'YqxkjQuestion.aspx?memberId=' + mid + '&fromfree=nofree&source=khlx&courseId=' + $("#hid_goodsId").val() + '&r=' + Math.random();
        }
        return false;
    }
    ...
    function add_comment() {
        if ($("#txt_msg").val() == "") { layer.msg("请输入你的评论"); return false; }
        $.ajax({
            type: "get",
            url: "YqxkjMemberInterface.ashx",
            data: { flag: "add_comment", goodsid: $("#hid_goodsId").val(), msg: escape($("#txt_msg").val()), truename: escape("<%=this.TrueName %>") },
            dataType: "text",
            async: true,
            success: function (data) {
                layer.msg("提交成功，感谢你的评论");
                $('#makeComment').hide();
                $('body').css('position', 'relative');
            }
        });
