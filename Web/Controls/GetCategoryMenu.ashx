...
public class GetCategoryMenu : IHttpHandler {
    ...
    void GetNowCategory(HttpContext context)
    {
        ...
        {
            strSQL += "select categoryID,categoryName from T_Menu_Category where pcategoryID='" + strPath[i] + "';";
        }
        string check = "";
    ...
    {
        string strSQL = "select * from T_Menu_Category where pcategoryid='" + StarTech.KillSqlIn.Form_ReplaceByString(context.Request.Params["categoryID"], 10) + "'";
        DataSet ds=adohelper.ExecuteSqlDataset(strSQL);
        if (ds == null || ds.Tables.Count < 1 || ds.Tables[0].Rows.Count < 1)
        {
