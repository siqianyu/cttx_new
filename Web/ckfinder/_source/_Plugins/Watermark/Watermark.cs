...
namespace CKFinder.Plugins
{
	public class Watermark : CKFinder.CKFinderPlugin
	{
		...
		public void Init(CKFinder.Connector.CKFinderEvent CKFinderEvent)
		{
            ...
            if (!CKFinder.Connector.Config.Current.PluginSettings.ContainsKey("Watermark_source")) CKFinder.Connector.Config.Current.PluginSettings["Watermark_source"] = "logo.png";
		...
		private void CreateWatermark(string sourceFile)
		{
            ...
            if (logoImage.Width * 4 > sourceImage.Width || logoImage.Height * 4 > sourceImage.Height)
				bitmapImage = resizeImage(logoImage,
					new Size(logoImage.Width *4 > sourceImage.Width ? sourceImage.Width/4 : logoImage.Width,
						logoImage.Height*4 > sourceImage.Height ? sourceImage.Height/4 : logoImage.Height));
			...
			bitmapImage.Dispose();
			graphics.Dispose();
			destinationBmp.Save(sourceFile);
			sourceImage.Dispose();
		}
