                ...
                if (oFile != null)
                {
                    ...
                    if (Connector.CheckFileName(sFileName) && !Config.Current.CheckIsHiddenFile(sFileName))
                    {
                        ...
                        if (Config.Current.ForceSingleExtension)
                        {
                            ...
                            string UpLoadFileTime = DateTime.Now.Year + DateTime.Now.ToString("HHmmss") + rnd.Next(9999).ToString("0000"); //生成一个新的数图片名称
                            string sExt = System.IO.Path.GetExtension(sFileName);
                            sFileName = Regex.Replace(UpLoadFileTime + sExt, @"\.(?![^.]*$)", "_", RegexOptions.None);
                        }
