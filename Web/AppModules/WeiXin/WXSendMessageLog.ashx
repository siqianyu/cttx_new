<%@ WebHandler Language="C#" Class="WXSendMessageLog" %>
...
public class WXSendMessageLog : IHttpHandler
{
    ...
    {
        string table = " (select sysnumber, a.WXOpenId, type, createTime, title, remark,b.TrueName from T_WXSendMessage_Log a left join T_Member_Info b on a.WXOpenId=b.WXOpenId) vv";
        //string fields = "AdId,title,Link,DisplayModeName,CategoryName,sort";//字段顺序和必须前台jggrid设置的一样
        string fields = "sysnumber, TrueName, type, createTime, title, remark,'操作' as cmd_col ";//字段顺序和必须前台jggrid设置的一样
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        if (hTable != null && hTable.Count > 0)
        {
            ...
            if (hTable.Contains("createTime") && hTable["createTime"].ToString().Trim() != "")
            {
                filter += " and createTime >= '" + hTable["createTime"].ToString().Trim() + "'";
            }
        }
        return filter;
    }
