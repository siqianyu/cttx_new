<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MenuList.aspx.cs" Inherits="AppModules_Menu_MenuList" %>
        ...
        <table>
            <tr>
                <td style=" width:80%">
                    ...
                    <div class="spanSearch" >菜名：</div>
                    <input style="width: 120px;" id="txtMenuName" />
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'Menu.ashx?flag=list<%=categoryId %>',
        datatype: "json",
        colNames: ['菜谱Id', '预览图', '菜名', '工艺', '口感', '烹饪时间', '卡路里', '添加用户', '添加时间', '是否显示', '','分量', '操作'],
        ...
        colModel: [
            ...
            { name: 'imgSrc', index: 'imgSrc', width: 80, align: 'center', classes: 'goodsimgstyle' },
            { name: 'menuName', index: 'menuName', width: 100, align: 'left' },
   		    { name: 'Technology', index: 'Technology', width: 50, align: 'center' },
            { name: 'Flavor', index: 'Flavor', width: 30, align: 'center' },
            { name: 'CookingTime', index: 'CookingTime', width: 30, align: 'center' },
            { name: 'Calorie', index: 'Calorie', width: 30, align: 'center' },
            { name: 'UserId', index: 'UserId', width: 50, align: 'center', hidden: true },
            { name: 'AddTime', index: 'AddTime', width: 30, align: 'center' },
            { name: 'isShow', index: 'isShow', width: 30, align: 'center' },
            { name: 'orderby', index: 'orderby', width: 30, align: 'center', hidden: true },
            { name: 'unit', align: 'center', width: 80,hidden:true },
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "menuId");
            }
        },
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    var id = rowData.menuId;
                    var imgSrc = rowData.imgSrc;
                    var _isShow = rowData.isShow;
                    var writeData = {
                        imgSrc: "<img src='" + imgSrc + "' width='70' height='70'/>",
                        isShow: _isShow == "1" ? "<font color='green'>显示</font>" : "<font color='red'>隐藏</font>",
    ...
    function grid_search() {
        ...
        var _AreaName = $("#txtMenuName").val();
        //var _Pid = $("#ddlPid").val();
        var _searchfilter = "menuName$$" + _AreaName; //+ "_$$_AreaId$$" + _AreaId + "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "Menu.ashx?flag=list<%=categoryId %>&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddMenu.aspx?r=' + Math.random() },
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "Menu.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
