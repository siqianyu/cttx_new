...
public partial class Member_TestDetailByPrint : System.Web.UI.Page
{
    ...
    {
        //AjaxPro.Utility.RegisterTypeForAjax(typeof(Member_TestDetailByPrint));
    ...
    public string CreateCommon(string questionType)
    {
        ...
        if (questionType == "是非题") { this.pd_num = rowItems.Length.ToString(); }
        else if (questionType == "A1型题,A2型题") { this.dx_num = rowItems.Length.ToString(); }
        else if (questionType == "X型题") { this.fx_num = rowItems.Length.ToString(); }
        ...
        if (rowItems.Length > 0)
        {
            for (int i = 0; i < rowItems.Length; i++)
            {
                ...
                html += "<p><em id='em_" + rowItems[i]["sysnumber"].ToString() + "' style=\"font-style: normal; color: blue; font-family: '微软雅黑'\"><b>" + (i + 1) + "：</b>" + rowItems[i]["questionTitle"] + "</em></p>";
                html += GetOptions(rowItems[i]["sysnumber"].ToString(), ((rowItems[i]["questionType"].ToString() == "X型题") ? "checkbox" : "radio"));
            }
        }
        else
        {
    ...
    public string CreateAL()
    {
        string sysNums = GetQuestions(this.tesSys);
        string html = "";
        DataTable dt = new BllTestQueston().GetList("sysnumber in('" + sysNums + "') and questionTitle<>'' and questionType in('A3/A4型题')").Tables[0];
        ...
        if (rowHeaders.Length > 0)
        {
            ...
            {
                sn++;
                html += "<p style='padding-top:10px;padding-bottom:10px;'><em style=\"font-style: normal; color: red; font-family: '微软雅黑'\"><b>" + sn + "：</b>" + rowHeader["questionTitle"] + "</em></p>";
