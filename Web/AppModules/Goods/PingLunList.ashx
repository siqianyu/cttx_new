<%@ WebHandler Language="C#" Class="PingLunList" %>
...
public class PingLunList : IHttpHandler
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        string goodsid = context.Request["goodsid"] == null ? "" : KillSqlIn.Url_ReplaceByString(context.Request.QueryString["goodsid"].ToLower(), Int32.MaxValue);
        ...
        if (flag == "list")
        {
            context.Response.Write(List(page, rows, sidx, sord, searchfilter, goodsid));
        }
        else if (flag == "delete")
        {
        ...
        {
            context.Response.Write(ApproveData(id, context));
        }
    ...
    {
        string table = " (select sysnumber,goodsId, memberName, memberContent, addTime,isShow from T_Goods_PingLun) vv";

        string fields = "sysnumber, memberName, memberContent, addTime, isShow,'操作' as cmd_col ";//字段顺序和必须前台jggrid设置的一样
        string filter = GetFilter(searchFilter) + " and goodsId='" + goodsid + "'";
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        if (hTable != null && hTable.Count > 0)
        {
            ...
            {
                filter += " and memberName like '%" + hTable["memberName"].ToString().Trim() + "%'";
            }
        }
        return filter;
    }
    ...
    {
        int r = adoHelper.ExecuteSqlNonQuery("delete T_Goods_PingLun where sysnumber='" + ids + "'");
        return r > 0 ? "true" : "false";
    }

    public string ApproveData(string ids, HttpContext context)
    {
        int r = adoHelper.ExecuteSqlNonQuery("update T_Goods_PingLun set isShow=1  where sysnumber='" + ids + "'");
        return r > 0 ? "true" : "false";
    }
