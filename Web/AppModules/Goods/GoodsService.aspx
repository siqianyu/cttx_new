<%@ Page Language="C#" AutoEventWireup="true" CodeFile="GoodsService.aspx.cs" Inherits="AppModules_Goods_GoodsService" %>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'Service.ashx?flag=list',
        datatype: "json",
        colNames: ['编号', '服务内容', '服务说明', '可选项',  '操作'],
        colModel: [
        //serviceId,serviceName,serviceContext,'选项' as serviceValue,'操作' as cmd_col
            {name: 'serviceId', index: 'GoodsId', hidden: true },
            { name: 'serviceName', index: 'CategoryName', align: 'left' },
   		    { name: 'serviceContext', index: 'GoodsName', align: 'left' },
            { name: 'serviceValue', index: 'GoodsCode', align: 'center' },
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "srviceId");
            }
        },
    ...
    function grid_search() {
        ...
        //var _Pid = $("#ddlPid").val();
        var _searchfilter = "serviceName$$" + _AreaName; //+ "_$$_AreaId$$" + _AreaId + "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "Service.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'ServiceAdd.aspx?r=' + Math.random() },
            area: [document.body.scrollWidth - 20, 500],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'ServiceAdd.aspx?id=' + id },
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "Service.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
    ...
    function show_method(id) {
        $.layer({
            ...
            title: ['查看', true],
            maxmin: true,
            iframe: { src: 'ServiceAdd.aspx?rd=1&id=' + id },
            area: ['1000px', '550px'],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
