<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PingLunList.aspx.cs" Inherits="AppModules_IACenter_PingLunList" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch" style="width:260px;display:none;"><div class="spanSearch" >订单编号：</div><input style="width: 170px;" id="txtOrderId" /></div>
                    <div class="divSearch"><div class="spanSearch" >接收人：</div><input style="width: 100px;" id="txtName" /></div>
                </td>
                <td style=" width:20%">
...
<script language="javascript" type="text/javascript">
    //sysnumber, memberName, memberContent, addTime, isShow
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'PingLunList.ashx?flag=list&goodsid=<%=this.goodsid %>',
        datatype: "json",
        colNames: ['编号', '评论人', '内容', '时间', '是否显示', '操作'],
        colModel: [
            { name: 'sysnumber',  hidden: true },
            { name: 'memberName', index: 'memberName', width: 50 },
            { name: 'memberContent', index: 'memberContent', width: 190 },
   		    { name: 'addTime', index: 'addTime', width: 50, align: 'center' },
            { name: 'isShow', index: 'isShow', width: 50 , align: 'center'},
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    ...
                    var writeData = { cmd_col: "<input type='button' class='CommonButon' value='审核' onclick=\"button_actions('approve','" + id + "')\"> <input type='button' class='CommonButon' value='删除' onclick=\"button_actions('delete','" + id + "')\">" }
    ...
    function approve_method(id) {
        layer.confirm('确定要审核通过吗?', function () {
            var str = $.ajax({ url: "PingLunList.ashx?flag=approve&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("审核成功！");
            }
            else {
    ...
    function grid_search() {
        ...
        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "MessageLog.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "PingLunList.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
