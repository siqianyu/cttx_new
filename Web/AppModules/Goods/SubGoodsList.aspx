<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SubGoodsList.aspx.cs" Inherits="AppModules_Goods_GoodsList" %>
            ...
            <tr>
                ...
                <td style=" width:20%">
                    <div class="divSearchUC"><input type="button" class="SelectBut" value="查&nbsp;询" title="查询" onclick="grid_search()" />
                     ...
                     <asp:Button ID="btnAdd" runat="server" Text="添加任务" OnClientClick="button_actions('add'); return false;" CssClass="Green InputClass"/>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'SubGoodsHandler.ashx?flag=list&pgoodsid=' + $("#hid_pgoodsid").val() + '',
        datatype: "json",
        colNames: ['序号', '所属课程', '预览图片', '任务名称', '播放数量', '价格', '施工地点', '添加时间', '是否发布', '排序', '操作', ''],
        colModel: [
            { name: 'GoodsId', index: 'GoodsId', hidden: true },
            { name: 'CategoryId', index: 'CategoryName', width: 60, align: 'left' },
            ...
            { name: 'TotalSaleCount', index: 'TotalSaleCount', width: 50, align: 'center' },
            { name: 'SalePrice', index: 'SalePrice', width: 50, align: 'center', hidden: true },
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    ...
                    var writeData = {
                        CategoryId: cName,
                        isSale: issale,
                        GoodsSmallPic:"<img src='"+imgSrc+"' width='80' height='60'/>",
                        cmd_col: "<input type='button' class='CommonButon' value='编辑' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='评论' onclick=\"button_actions('pinglun','" + id + "')\"> <input type='button' class='CommonButon' value='组卷' onclick=\"button_actions('shiti','" + id + "')\"> <input type='button' class='CommonButon' value='删除' onclick=\"button_actions('delete','" + id + "')\">"
                    }
    ...
    function button_actions(flag, id, lang) {
        ...
        if (flag == "add") {
            ...
            location.href = "GoodsList.aspx?categoryId=" + $("#hid_categoryId").val();
            return false;
        } 
            ...
            location.href = "../Examination/CouseTestZX.aspx?categoryId=" + $("#hid_categoryId").val() + "&pgoodsId="+$("#hid_pgoodsid").val()+"&goodsId=" + id;
        }
    ...
    function grid_search() {
        ...
        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "SubGoodsHandler.ashx?flag=list&pgoodsid=" + $("#hid_pgoodsid").val() + "&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'SubGoodsAdd.aspx?pgoodsid=' + $("#hid_pgoodsid").val() },
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'SubGoodsAdd.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
