<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SignList.aspx.cs" Inherits="AppModules_Goods_SignList" %>
        ...
        <table>
            <tr>
                <td style=" width:80%">
                    ...
                    <div class="spanSearch" >标签名：</div>
                    <input style="width: 100px;" id="txtSignName" />
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'SignList.ashx?flag=list',
        datatype: "json",
        colNames: ['标签编号', '标签名字', '备注', '添加时间','操作'],
        colModel: [
            { name: 'signId', index: 'signId'},
            { name: 'signName', index: 'signName'},
            { name: 'remark', index: 'remark' },
            { name: 'addTime', index: 'addTime'},
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "signId");
            }
        },
    ...
    function button_actions(flag, id, lang) {
        ...
        if (flag == "add") {
            ...
            var str = $.ajax({ url: "SignList.ashx?flag=delete&lang='" + lang + "'id=" + id + "", async: false }).responseText;
        }
        else if (flag == "delete") {
            delete_method(id);
        }
    ...
    function grid_search() {
        ...
        var _AreaName = $("#txtSignName").val();
        //var _Pid = $("#ddlPid").val();
        var _searchfilter = "SignName$$" + _AreaName; //+ "_$$_AreaId$$" + _AreaId + "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "SignList.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'SignAdd.aspx?r=' + Math.random() },
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "SignList.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
