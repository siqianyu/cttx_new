<%@ WebHandler Language="C#" Class="SubGoodsHandler" %>
...
public class SubGoodsHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        string pgoodsid = context.Request["pgoodsid"] == null ? "" : KillSqlIn.Url_ReplaceByString(context.Request.QueryString["pgoodsid"], Int32.MaxValue);   //排序规则

        if (flag == "list")
        {
            context.Response.Write(List(page, "10", sidx, sord, searchfilter, pgoodsid));
        }
        else if (flag == "delete")
        {
    ...
    public string Delete(string goodsid)
    {
        ...
        if (row > 0)
        {
            LogAdd.CreateLog(HttpContext.Current.Session["UserId"].ToString(), "删除任务《" + GoodsName + "》;", "删除", "", "", HttpContext.Current.Request.Url.ToString());
    ...
    /// <returns></returns>
    public string List(string curPage, string pageSize, string orderBy, string orderByRole, string searchFilter, string pgoodsid)
    {
        string table = " (select a.* from T_Goods_Info a) vv";
        string fields = "GoodsId,GoodsToTypeId,GoodsSmallPic,GoodsName,TotalSaleCount,SalePrice,JobAddress,AddTime,IsSale,orderby, '操作' as cmd_col, '分类名' as CategoryName,uint";//字段顺序和必须前台jggrid设置的一样
        string filter = GetFilter(searchFilter);
        filter += " and GoodsToTypeId ='" + pgoodsid + "' and JobType='SubGoods'";
    ...
    public void EditDataSource(ref DataTable dt)
    {
        foreach (DataRow dr in dt.Rows)
        {
            ...
            DataSet ds = adoHelper.ExecuteSqlDataset("select GoodsName from T_Goods_Info where GoodsId='" + dr["GoodsToTypeId"] + "'");
            dr["CategoryName"] = ds.Tables[0].Rows.Count > 0 ? ds.Tables[0].Rows[0]["GoodsName"] : "";
        }
    }
