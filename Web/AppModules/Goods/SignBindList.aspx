<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SignBindList.aspx.cs" Inherits="AppModules_Goods_SignBindList" %>
...
<script language="javascript" type="text/javascript">
    ...
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'SignList.ashx?flag=bindlist',
        datatype: "json",
        colNames: ['任务id', '任务名字', '任务编号', '标签列表', '操作'],
        colModel: [
            { name: 'goodsId', index: 'goodsId' },
            { name: 'goodsName', index: 'goodsName' },
            { name: 'goodsCode', index: 'goodsCode' },
            { name: 'sign', index: 'sign' },
    ...
    function button_actions(flag, id, lang) {
        ...
        if (flag == "add") {
            ...
            var str = $.ajax({ url: "SignList.ashx?flag=binddelete&lang='" + lang + "'id=" + id + "", async: false }).responseText;
        }
        else if (flag == "delete") {
            delete_method(id);
        }
    ...
    function grid_search() {
        ...
        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "SignList.ashx?flag=bindlist&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'SignBindAdd.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'SignBindAdd.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
