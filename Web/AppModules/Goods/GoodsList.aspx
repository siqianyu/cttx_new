<%@ Page Language="C#" AutoEventWireup="true" CodeFile="GoodsList.aspx.cs" Inherits="AppModules_Goods_GoodsList" %>
        ...
        <table>
            <tr>
                <td style=" width:80%">
                    ...
                    <div class="spanSearch" >课程名：</div>
                    <input style="width: 150px;" id="txtGoodsName" />
                ...
                <td style=" width:20%">
                 ...
                 <asp:Button ID="btnOrder" runat="server" Text=" 人工下单 "  OnClientClick="return button_actions('addorder')"  CssClass="DarkRed InputClass" />
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'GoodsHandler.ashx?flag=list<%=categoryId %>',
        datatype: "json",
        colNames: ['序号', '分类', '预览图片', '课程名称', '任务数', '价格','是否发布', '施工地点', '添加时间', '开始时间', '完成时间', '', '操作', ''],
        colModel: [
            { name: 'GoodsId', index: 'GoodsId', hidden: true },
            { name: 'CategoryId', index: 'CategoryName', width: 50, align: 'left' },
            { name: 'GoodsSmallPic', index: 'GoodsSmallPic', width: 60, align: 'center',classes:'goodsimgstyle' },
   		    { name: 'GoodsName', index: 'GoodsName', width: 150, align: 'left' },
            { name: 'PLCount', index: 'PLCount', width: 40, align: 'center' },
            { name: 'SalePrice', index: 'SalePrice', width: 50, align: 'center' },
            { name: 'IsSale', index: 'IsSale', width: 40, align: 'center' },
            { name: 'JobAddress', index: 'JobAddress', width: 100, align: 'left', hidden: true },
            { name: 'AddTime', index: 'AddTime', width: 60, align: 'center' },
            { name: 'JobStartTime', index: 'JobStartTime', width: 60, align: 'center', hidden: true },
            { name: 'JobEndTime', index: 'JobEndTime', width: 60, align: 'center', hidden: true },
            { name: 'orderby', index: 'orderby', width: 60, align: 'center', hidden: true },
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    ...
                    var issale = rowData.isSale == 1 ? "<font color='green'>销售中</font>" : "<font color='red'>已下架</font>";
                    var imgSrc = rowData.GoodsSmallPic;
                    var writeData = {
                        CategoryId: cName,
                        isSale: issale,
                        GoodsSmallPic:"<img src='"+imgSrc+"' width='100' height='60'/>",
                        cmd_col: "<input type='button' class='CommonButon' value='任务管理' onclick=\"button_actions('task','" + id + "')\"> <input type='button' class='CommonButon' value='编辑' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='删除' onclick=\"button_actions('delete','" + id + "')\">"
                    }
    ...
    function button_actions(flag, id, lang) {
        ...
        if (flag == "add") {
            ...
            location.href = "SubGoodsList.aspx?categoryId=<%=Request["categoryId"] %>&pgoodsId=" + id;
        }else if (flag == "pinglun") {
            location.href = "PingLunList.aspx?goodsId=" + id;
        }
        else if (flag == "delete") {
            delete_method(id);
        }
    ...
    function grid_search() {
        ...
        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "GoodsHandler.ashx?flag=list<%=categoryId %>&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function orderAll() {
        ...
        if (!IsCheck) {
            ...
            for (var i = 0; i < rowData.length; i++) {
                id += jQuery('#startech_table_jqgrid').jqGrid('getCell', rowData[i], 'GoodsId') + ","; //获取所有选中的id值
            }
            $.layer({
                type: 2,
                shade: [0.1, '#000'],
                ...
                title: ['人工下单', true],
                maxmin: true,
                iframe: { src: '../Order/CreateOrderFree.aspx?GoodsIds=' + id },
                area: ['660px', '580px'],
                offset: ['0px', ''],
                close: function (index) {
                    //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
