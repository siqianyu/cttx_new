<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MorePropertySetList.aspx.cs" Inherits="AppModules_Goods_MorePropertySetList" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">

<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>属性列表-才通天下微信公号后台管理</title>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch"><div class="spanSearch" >属性名：</div><input style="width: 100px;" id="txtFormateName" /></div>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'PropertySetHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['序号', '属性名称', '属性值', '类型', '', '操作'],
        colModel: [
            { name: 'propertyId', index: 'GoodsId', hidden: true },
            { name: 'propertyName', index: 'CategoryName', width: 80, align: 'left' },
   		    { name: 'propertyOptions', index: 'GoodsName', width: 100, align: 'left' },
            { name: 'porpertyFlag', index: 'GoodsCode', width: 30, align: 'center' },
            { name: 'orderby', index: 'CategoryId', width: 50, align: 'center', hidden: true },
        ...
        gridComplete: function () {
            ...
            $("[title=select]").text("可选择");
            $("[title=input]").text("仅显示");
            for (var i = 0; i < ids.length; i++) {
                var rowData = $("#startech_table_jqgrid").getRowData(ids[i]);
                if (rowData) {
                    var id = rowData.propertyId;
    ...
    function grid_search() {
        ...
        var _AreaName = $("#txtFormateName").val();
        //var _Pid = $("#ddlPid").val();
        var _searchfilter = "propertyName$$" + _AreaName; //+ "_$$_AreaId$$" + _AreaId + "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "PropertySetHandler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'PropertySetAdd.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'PropertySetAdd.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "PropertySetHandler.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
    ...
    function show_method(id) {
        $.layer({
            ...
            title: ['查看', true],
            maxmin: true,
            iframe: { src: 'PropertySetAdd.aspx?rd=1&id=' + id },
            area: ['1000px', '550px'],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
