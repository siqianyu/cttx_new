<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PayLogList.aspx.cs" Inherits="AppModules_IACenter_PayLogList" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch" style="width:260px;"><div class="spanSearch" >订单编号：</div><input style="width: 170px;" id="txtOrderId" /></div>
                    <div class="divSearch"><div class="spanSearch" >支付人：</div><input style="width: 100px;" id="txtName" /></div>
                    <div class="divSearch" style="width:160px;"><span class="spanSearch">支付时间：</span>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'PayLog.ashx?flag=list',
        datatype: "json",
        colNames: ['订单编号', '支付金额', '支付时间', '订单类型', '下单时间', '支付人', '电话', '订单描述', '操作'],
        colModel: [
            { name: 'orderId', index: 'orderId', width: 70, align: 'center' },
            { name: 'paymoney', index: 'paymoney', width: 40, align: 'center' },
            { name: 'payTime', index: 'payTime', width: 80, align: 'center' },
   		    { name: 'OrderType', index: 'OrderType', width: 50, align: 'center' },
            { name: 'OrderTime', index: 'OrderTime', width: 80, align: 'center' },
            { name: 'ReceivePerson', index: 'ReceivePerson', width: 60, align: 'center' },
            { name: 'Tel', index: 'Tel', width: 60, align: 'center' },
            { name: 'payGoodsDetail', index: 'payGoodsDetail',  align: 'left' },
    ...
    function grid_search() {
        ...
        var _orderId = $("#txtOrderId").val();
        var _txtDateBegin = $("#txtDateBegin").val();
        var _txtDateEnd = $("#txtDateEnd").val();
        var _searchfilter = "ReceivePerson$$" + _txtName + "_$$_orderId$$" + _orderId + "_$$_txtDateBegin$$" + _txtDateBegin + "_$$_txtDateEnd$$" + _txtDateEnd;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "PayLog.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
