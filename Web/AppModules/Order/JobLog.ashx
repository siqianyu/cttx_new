<%@ WebHandler Language="C#" Class="JobLog" %>
...
public class JobLog : IHttpHandler {
    ...
    {
        string table = " T_Member_JobRecord ";
        //string fields = "AdId,title,Link,DisplayModeName,CategoryName,sort";//字段顺序和必须前台jggrid设置的一样
        string fields = "RecordId, MemberId, RecordTime, GoodsId, JobAddressCode, JobAddress, RecordInfo,'操作' as cmd_col ";//字段顺序和必须前台jggrid设置的一样
        ...
        if (orderBy.Equals("cmd_col")) { sort = " order by RecordTime desc "; }
        ...
        return JSONHelper.ToJGGridJson(curPage, totalPages.ToString(), totalRecords.ToString(), dtSource, fields.Replace("'操作' as ", "").Split(','), "RecordId");
    }
