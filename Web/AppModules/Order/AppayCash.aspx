<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AppayCash.aspx.cs" Inherits="AppModules_IACenter_AppayCash" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch" style="width:260px;"><div class="spanSearch" >会员号：</div><input style="width: 170px;" id="txtMemberId"  runat="server"/></div>
                    <div class="divSearch"><div class="spanSearch" >姓名：</div><input style="width: 100px;" id="txtName" runat="server" /></div>
                    <div class="divSearch" style="width:160px;"><span class="spanSearch">申请时间：</span>
    ...
    <div class="SosoBar Left" runat="server" id="div_total" visible="false">
        <table>
            <tr>
             <td style="font-size:14px;">&nbsp;金额总数：<span id="span_total" runat="server">0.00</span></td>
             <td style=" width:20%">
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'AppayCash.ashx?flag=list&showtype=<%=this.showtype %>',
        datatype: "json",
        colNames: ['sysnumber', '会员号', '姓名', '卡号', '金额', '申请时间', '处理状态', '处理备注', '操作'],
        colModel: [
            { name: 'Sysnumber', index: 'Sysnumber', width: 70, hidden:true },
            { name: 'MemberId', index: 'MemberId', width: 60, align: 'center' },
            { name: 'TrueName', index: 'TrueName', width: 60, align: 'center' },
   		    { name: 'CardNumber', index: 'CardNumber', width: 110, align: 'center' },
            { name: 'Moeny', index: 'Moeny', width: 60, align: 'center' },
            { name: 'ApplayTime', index: 'ApplayTime', width: 80, align: 'center' },
            { name: 'IfDeal', index: 'IfDeal', width: 60, align: 'center' },
            { name: 'DealRemarks', index: 'DealRemarks', align: 'left' },
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    ...
                    var writeData = { cmd_col: "<input type='button' class='CommonButon' value='处理' onclick=\"button_actions('edit','" + id + "')\">" }
    ...
    function grid_search() {
        ...
        var _searchfilter = "TrueName$$" + _txtName + "_$$_MemberId$$" + _MemberId + "_$$_txtDateBegin$$" + _txtDateBegin + "_$$_txtDateEnd$$" + _txtDateEnd;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "AppayCash.ashx?flag=list&showtype=<%=this.showtype %>&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
        gettotalmoney();
    }
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['详情信息', true],
            maxmin: true,
            iframe: { src: 'AppayCashDetail.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function gettotalmoney() {
        ...
        var _txtDateEnd = $("#txtDateEnd").val();
        var _searchfilter = "TrueName$$" + _txtName + "_$$_MemberId$$" + _MemberId + "_$$_txtDateBegin$$" + _txtDateBegin + "_$$_txtDateEnd$$" + _txtDateEnd;
        _searchfilter = escape(_searchfilter);
        //alert(_searchfilter);
        $.ajax({
            type: "get",
            url: "AppayCash.ashx?flag=totalmoney&showtype=<%=this.showtype %>",
            data: { searchfilter: _searchfilter },
            dataType: "text",
            async: false,
