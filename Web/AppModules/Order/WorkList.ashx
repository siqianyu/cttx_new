...
public class WorkList : IHttpHandler
{
    ...
    {
        string table = " V_WorkInfo ";
        //string fields = "AdId,title,Link,DisplayModeName,CategoryName,sort";//字段顺序和必须前台jggrid设置的一样
        string fields = "OrderId, GoodsSmallPic, GoodsName, JobAddress, JobStartTime, JobEndTime, JobDay,JobStatus,MemberId,isXP,'操作' as cmd_col ";//字段顺序和必须前台jggrid设置的一样
        ...
        if (orderBy.Equals("cmd_col")) { sort = " order by JobStartTime desc "; }
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        string filter = " 1=1 ";
        if (this.showtype == "1") { filter += " and JobStatus='已结束'"; }
        else if (this.showtype == "0") { filter += " and JobStatus='进行中'"; }
    ...
    public void EditDataSource(ref DataTable dt)
    {
        ...
        {
            object objName = adoHelper.ExecuteSqlScalar("select truename from t_member_info where memberid='" + dr["MemberId"] + "'");

            dr["MemberId"] = objName == null ? "" : objName.ToString();
            if (dr["isXP"].ToString() == "1") { dr["GoodsName"] = dr["GoodsName"].ToString() + "【续聘】"; }
        }
    }
