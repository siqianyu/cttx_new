...
public class PayLog : IHttpHandler
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        this.showtype = context.Request["showtype"] == null ? "" : KillSqlIn.Url_ReplaceByString(context.Request.QueryString["showtype"].ToLower(), Int32.MaxValue);
        ...
        else if (flag == "totalmoney")
        {
            context.Response.Write(GetTotalMoney(searchfilter));
        }
    ...
    {
        string table = " T_Member_ApplayCashBank ";
        //string fields = "Sysnumber,title,Link,DisplayModeName,CategoryName,sort";//字段顺序和必须前台jggrid设置的一样
        string fields = "Sysnumber,MemberId,TrueName,CardNumber,Moeny,ApplayTime,IfDeal,DealRemarks,IsPayByUnline,'操作' as cmd_col ";//字段顺序和必须前台jggrid设置的一样
        ...
        if (orderBy.Equals("cmd_col")) { sort = " order by ApplayTime desc "; }
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        if (hTable != null && hTable.Count > 0)
        {
            ...
            if (hTable.Contains("MemberId") && hTable["MemberId"].ToString().Trim() != "")
            {
                filter += " and MemberId in (select memberid from t_member_info where membername like '%" + hTable["MemberId"].ToString().Trim() + "%' )";
            }
            if (hTable.Contains("txtDateBegin") && hTable["txtDateBegin"].ToString().Trim() != "") { filter += " and ApplayTime >= '" + hTable["txtDateBegin"].ToString() + "'"; }
            if (hTable.Contains("txtDateEnd") && hTable["txtDateEnd"].ToString().Trim() != "") { filter += " and ApplayTime < '" + DateTime.Parse(hTable["txtDateEnd"].ToString()).AddDays(1) + "'"; }
    ...
    public void EditDataSource(ref DataTable dt)
    {
        foreach (DataRow dr in dt.Rows)
        {
            ...
            string IfDeal = dr["IfDeal"].ToString();
            if (IfDeal == "0") dr["IfDeal"] = "未处理";
            if (IfDeal == "-1") { dr["IfDeal"] = "未通过"; }
            if (IfDeal == "1") { dr["IfDeal"] = "已通过"; }
            if (IfDeal == "1" && dr["IsPayByUnline"].ToString() == "1") { dr["IfDeal"] = "已支付"; }
        }
    }
    ...
    protected string GetTotalMoney(string searchFilter)
    {
        AdoHelper adoHelper = AdoHelper.CreateHelper("DB_Instance");
        string filter = GetFilter(searchFilter);
        string sql = "select sum(Moeny) from T_Member_ApplayCashBank where " + filter;
