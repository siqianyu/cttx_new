...
public class Log : IHttpHandler {
    ...
    {
        string table = " IACenter_UserActionLog ";
        //string fields = "AdId,title,Link,DisplayModeName,CategoryName,sort";//字段顺序和必须前台jggrid设置的一样
        string fields = "id,userName,userTrueName,menuNameLevel1,menuNameLevel2,menuNameLevel3,actionType,actionTicket,remarks1,remarks2,ip,url,addTime,'操作' as cmd_col ";//字段顺序和必须前台jggrid设置的一样
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        if (hTable != null && hTable.Count > 0)
        {
            ...
            if (hTable.Contains("UserName") && hTable["UserName"].ToString().Trim() != "")
            {
                SafeStr = KillSqlIn.Form_ReplaceByString(hTable["UserName"].ToString().Trim(), Int32.MaxValue);
                filter += " and UserName like '%" + SafeStr + "%'";
            }
            
            if (hTable.Contains("menuNameLevel1") && hTable["menuNameLevel1"].ToString().Trim() != "")
            {
                SafeStr = KillSqlIn.Form_ReplaceByString(hTable["menuNameLevel1"].ToString().Trim(), Int32.MaxValue);
                filter += " and menuNameLevel1 like '%" + SafeStr + "%'";
            }

            if (hTable.Contains("actionType") && hTable["actionType"].ToString().Trim() != "")
            {
                SafeStr = hTable["actionType"].ToString().Trim();
                filter += " and actionType = '" + SafeStr + "'";
            }
