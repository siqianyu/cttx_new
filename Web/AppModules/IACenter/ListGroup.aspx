<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ListGroup.aspx.cs" Inherits="AppModules_IACenter_ListGroup" %>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'ListGroupHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['编号', '用户组编号', '用户组名称', '操作'],
        colModel: [
            { name: 'id', hidden: true },
            { name: 'uniqueId', index: 'uniqueId', width: 80 },
   		    { name: 'groupName', index: 'groupName', width: 250 },
    ...
    function button_actions(flag, id, lang) {
        if (flag == "add") {
            ...
            var str = $.ajax({ url: "ListGroupHandler.ashx?flag=delete&lang='" + lang + "'id=" + id + "", async: false }).responseText;
        }
        else if (flag == "delete") {
            delete_method(id);
        }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddGroup.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'AddGroup.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "ListGroupHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
            jQuery("#startech_table_jqgrid").trigger("reloadGrid");
    ...
    function show_method(id) {
        $.layer({
            ...
            title: ['查看', true],
            maxmin: true,
            iframe: { src: 'AddGroup.aspx?rd=1&id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function download_method(id, lang) {
        $.layer({
            ...
            title: ['查看', true],
            maxmin: true,
            iframe: { src: 'ListGroupHandler.ashx?lang=' + lang + '&id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
