<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ExaminationReport.aspx.cs" Inherits="AppModules_IACenter_ExaminationReport" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch"><div class="spanSearch" >姓名：</div><input style="width: 100px;" id="txtName" /></div>
                    <div class="divSearch"><div class="spanSearch" >任务：</div><input style="width: 100px;" id="txtGoodsName" />
                    </div>
                </td>
                <td style=" width:20%">
...
<script language="javascript" type="text/javascript">
    //MemberId,TrueName,Tel,MemberName,CourseId,GoodsName,CreateTime,TotalNum,TotalPass
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'SearchInteface.ashx?flag=list_examination',
        datatype: "json",
        colNames: ['MemberId','姓名', '电话', 'MemberName','CourseId', '任务','时间','总数','正确数', '操作'],
        colModel: [
            { name: 'MemberId', hidden: true },
            { name: 'TrueName', index: 'TrueName', width: 60, align: 'center' },
            { name: 'Tel', index: 'Tel', width: 80, align: 'center' },
   		    { name: 'MemberName', hidden: true  },
            { name: 'CourseId', hidden: true },
            { name: 'GoodsName', index: 'GoodsName', width: 120, },
            { name: 'CreateTime', index: 'CreateTime', width: 90, align: 'center' },
            { name: 'TotalNum', index: 'TotalNum', width: 50, align: 'center' },
            { name: 'TotalPass', index: 'TotalPass', width: 50, align: 'center' },
    ...
    function grid_search() {
       ...
       var _gname=$("#txtGoodsName").val();
        var _searchfilter = "TrueName$$" + _name + "_$$_GoodsName$$"+_gname;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "SearchInteface.ashx?flag=list_examination&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
