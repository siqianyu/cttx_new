<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FlagList.aspx.cs" Inherits="AppModules_Flag_FlagList" %>
                ...
                <tr>
                    <td style="width: 80%">
                        <div class="divSearch" style='width: 600px'>
                            <span class="spanSearch">标签名：</span>
                            <input style="width: 100px;" id="txtFlagName" />
                        </div>
                    </td>
                    <td style="width: 20%">
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'FlagHandler.ashx?flag=list<%=categoryId %>',
        datatype: "json",
        colNames: ['标签编号', '标签名称', '禁用状态', '操作'],
        colModel: [
            { name: 'flag_id', index: 'flag_id', width: 60 },
            { name: 'flag_name', index: 'flag_name', width: 60, align: 'center' },
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "Flag_id");
            }
        },
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    var id = rowData.flag_id;
                    var uname = rowData.flag_name;
                    var usetxt = rowData.if_use == 1 ? "禁用" : "启用";
                    var isuse = rowData.if_use == 1 ? "<font color='green'>启用</font>" : "<font color='red'>禁用</font>";
                    var writeData = {
                        if_use: isuse,
                        //MemberStatus: MemberStatus,
                        cmd_col: "<input type='button' class='CommonButon' value='编辑' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='" + usetxt + "' onclick=\"button_actions('use" + rowData.if_use + "','" + id + "')\">"
                    }
    ...
    function grid_search() {
        ...
        var FlagName = $("#txtFlagName").val();
        //var tel = $("#txtTel").val();
        //var tname = $("#txtName").val();
        var _searchfilter = "flag_name$$" + FlagName;  //+ "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "FlagHandler.ashx?flag=list<%=categoryId %>&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddFlags.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        ...
        $.layer({
            ...
            title: ['详情', true],
            maxmin: true,
            iframe: { src: 'AddFlags.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(flag, id) {
        ...
        layer.confirm('确定要' + context + '吗?', function () {
            var str = $.ajax({ url: "FlagHandler.ashx?flag=use&id=" + id + "&statu=" + state + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert(context + "成功！");
            }
            else {
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var id = "";
                for (var i = 0; i < rowData.length; i++) {
                    id += jQuery('#startech_table_jqgrid').jqGrid('getCell', rowData[i], 'flag_id') + "|"; //获取所有选中的id值
                }
                //alert(id);
                var str = $.ajax({ url: "FlagHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
