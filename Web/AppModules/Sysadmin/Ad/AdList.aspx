<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AdList.aspx.cs" Inherits="AppModules_Sysadmin_Base_AdList" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch"><div class="spanSearch" >广告名称：</div><input style="width: 100px;" id="txtAdName" /></div>
                    <div class="divSearch"><div class="spanSearch" >广告地址：</div><input style="width: 100px;" id="txtAdLink" /></div>
                    <div class="divSearch"><div class="spanSearch" >广告类别：</div><asp:DropDownList ID="ddlAdType" runat="server" Width="102">
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'AdHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['序号','广告链接名称', '广告地址', '广告类型', '所属类别', '排序', '操作'],
        colModel: [
            { name: 'AdId', index: 'AdId',hidden: true  },
            { name: 'title', index: 'title', width: 80, align: 'left' },
   		    { name: 'Link', index: 'Link', width: 100, align: 'left' },
            { name: 'DisplayModeName', index: 'DisplayMode', width: 40, align: 'center' },
            { name: 'CategoryName', index: 'CategoryId', width: 50, align: 'center' },
    ...
    function grid_search() {
        var _AdName = $("#txtAdName").val();
        var _AdLink = $("#txtAdLink").val();
        var _AdType = $("#ddlAdType").val();
        var _searchfilter = "AdName$$" + _AdName + "_$$_AdLink$$" + _AdLink + "_$$_AdType$$" + _AdType;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "AdHandler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
