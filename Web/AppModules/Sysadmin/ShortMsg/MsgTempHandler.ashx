<%@ WebHandler Language="C#" Class="MsgTempHandler" %>
...
public class MsgTempHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public string Disable(string goodsid, string statu)
    {
        ...
        string strSQL = "update T_ShortMessage_Template set isUse='" + statu + "' where templateId='" + gid + "';";
    ...
    public string DeleteData(string ids, HttpContext context)
    {
        ...
        if (ids.IndexOf("|") < 0)
        {
            string strSQL = "delete from T_ShortMessage_Template where templateId='" + ids + "'";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        else
        {
            ids = ids.Replace("|", "','");
            string strSQL = "delete from T_ShortMessage_Template where templateId in('" + ids + "')";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        return result;
    }
    ...
    {
        string table = " T_ShortMessage_Template";
        string fields = "templateId,flag,supplierId,supplierFlag,templateCode,templateParam,isUse, '操作' as cmd_col";//字段顺序和必须前台jggrid设置的一样
        ...
        string sort = "order by " + orderBy + " " + orderByRole + ",templateId";
        ...
        return JSONHelper.ToJGGridJson(curPage, totalPages.ToString(), totalRecords.ToString(), dtSource, fields.Replace("'操作' as ", "").Split(','), "templateId");
    }
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        {
            string SafeStr = "";
            if (hTable.Contains("templateCode") && hTable["templateCode"].ToString().Trim() != "")
            {
                SafeStr = KillSqlIn.Form_ReplaceByString(hTable["templateCode"].ToString().Trim(), Int32.MaxValue);
                filter += " and templateCode like '%" + SafeStr + "%'";
            }
        }
        return filter;
    }
