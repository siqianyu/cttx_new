<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MsgSupList.aspx.cs" Inherits="AppModules_Sysadmin_ShortMsg_MsgSupList" %>
                ...
                <tr>
                    <td style="width: 20%">
                        <div class="divSearchUC">
                            <input type="button" style="display:none;" class="SelectBut" value="查&nbsp;询" title="查询" onclick="grid_search()" />
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'MsgSupHandler.ashx?flag=list<%=categoryId %>',
        datatype: "json",
        colNames: ['编号', '标识', '商家名称', "Key", "Secret", '签名', '排序', '启用', '默认', '操作'],
        colModel: [
            { name: 'supplierId', index: 'supplierId', width: 40 },
            { name: 'flag', index: 'flag', width: 40, align: 'center' },
            { name: 'supplierName', index: 'supplierName', width: 60, align: 'center' },
            { name: 'accessKeyID', index: 'accessKeyID', width: 60, align: 'center' },
            { name: 'accessKeySecret', index: 'accessKeySecret', width: 120, align: 'center' },
            { name: 'smsSign', index: 'smsSign', width: 40, align: 'center' },
            { name: 'sort', index: 'sort', width: 40, align: 'center' },
            { name: 'isUse', index: 'isUse', width: 40, align: 'center' },
            { name: 'isDefault', index: 'isDefault', width: 40, align: 'center' },
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "supplierid");
            }
        },
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddMsgSup.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        ...
        $.layer({
            ...
            title: ['详情', true],
            maxmin: true,
            iframe: { src: 'AddMsgSup.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(flag, id) {
        ...
        layer.confirm('确定要' + context + '吗?', function () {
            var str = $.ajax({ url: "MsgSupHandler.ashx?flag=use&id=" + id + "&statu=" + state + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert(context + "成功！");
            }
            else {
    ...
    function default_method(flag, id) {
        ...
        layer.confirm('确定要' + context + '吗?', function () {
            var str = $.ajax({ url: "MsgSupHandler.ashx?flag=default&id=" + id + "&statu=" + state + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert(context + "成功！");
            }
            else {
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var id = "";
                for (var i = 0; i < rowData.length; i++) {
                    id += jQuery('#startech_table_jqgrid').jqGrid('getCell', rowData[i], 'supplierId') + "|"; //获取所有选中的id值
                }
                //alert(id);
                var str = $.ajax({ url: "MsgSupHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
