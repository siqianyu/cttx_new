<%@ WebHandler Language="C#" Class="MsgSupHandler" %>
...
public class MsgSupHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {
            string statu = KillSqlIn.Form_ReplaceByString(context.Request.QueryString["statu"], 20);
            context.Response.Write(Default(id, statu));
        }
        else if (flag == "cz")
        {
    ...
    public string Disable(string goodsid, string statu)
    {
        ...
        string strSQL = "update T_ShortMessage_Supplier set isUse='" + statu + "' where supplierid='" + gid + "';";
    ...
    /// <param name="goodsid"></param>
    /// <returns></returns>
    public string Default(string goodsid, string statu)
    {
        string gid = KillSqlIn.Form_ReplaceByString(goodsid, 20);
        string strSQL = "update T_ShortMessage_Supplier set isDefault='" + statu + "' where supplierid='" + gid + "';";
        if (statu == "1")
        {
            strSQL += "update T_ShortMessage_Supplier set isDefault='0' where supplierid<>'" + gid + "';"; ;
        }
        int row = adoHelper.ExecuteSqlNonQuery(strSQL);
        if (row > 0)
            return "true";
    ...
    public string DeleteData(string ids, HttpContext context)
    {
        ...
        if (ids.IndexOf("|") < 0)
        {
            string strSQL = "delete from T_ShortMessage_Supplier where supplierId='" + ids + "'";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        else
        {
            ids = ids.Replace("|", "','");
            string strSQL = "delete from T_ShortMessage_Supplier where supplierId in('" + ids + "')";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        return result;
    }
    ...
    {
        string table = " T_ShortMessage_Supplier";
        string fields = "supplierId,flag,supplierName,accessKeyID,accessKeySecret,smsSign,sort,isUse,isDefault, '操作' as cmd_col";//字段顺序和必须前台jggrid设置的一样
        ...
        string sort = "order by " + orderBy + " " + orderByRole + ",isUse desc,sort desc";
        ...
        //if (orderBy.Equals("cmd_col")) { sort = " order by flag_id desc "; }
        ...
        return JSONHelper.ToJGGridJson(curPage, totalPages.ToString(), totalRecords.ToString(), dtSource, fields.Replace("'操作' as ", "").Split(','), "supplierId");
    }
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        {
            string SafeStr = "";
            if (hTable.Contains("supplierName") && hTable["supplierName"].ToString().Trim() != "")
            {
                SafeStr = KillSqlIn.Form_ReplaceByString(hTable["supplierName"].ToString().Trim(), Int32.MaxValue);
                filter += " and supplierName like '%" + SafeStr + "%'";
            }
        }
        return filter;
    }
