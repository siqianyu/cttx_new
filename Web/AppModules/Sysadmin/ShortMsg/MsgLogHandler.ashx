<%@ WebHandler Language="C#" Class="MsgLogHandler" %>
...
public class MsgLogHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public string DeleteData(string ids, HttpContext context)
    {
        ...
        if (ids.IndexOf("|") < 0)
        {
            string strSQL = "delete from T_ShortMessage_Log where sysnumber='" + ids + "'";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        else
        {
            ids = ids.Replace("|", "','");
            string strSQL = "delete from T_ShortMessage_Log where sysnumber in('" + ids + "')";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        return result;
    }
    ...
    {
        string table = " T_ShortMessage_Log";
        string fields = "sysnumber,tel,yzm,statu,sendTime,outSendTime,remark,template,smsSign,result,sendText,supplier";//字段顺序和必须前台jggrid设置的一样
