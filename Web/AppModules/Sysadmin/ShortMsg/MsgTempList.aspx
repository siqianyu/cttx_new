<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MsgTempList.aspx.cs" Inherits="AppModules_Sysadmin_ShortMsg_MsgTempList" %>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'MsgTempHandler.ashx?flag=list<%=categoryId %>',
        datatype: "json",
        colNames: ['编号', '标识', "供应商", '供应商标识', 'Code', "Param", '启用', '操作'],
        colModel: [
            { name: 'templateId', index: 'templateId', width: 40 },
            { name: 'flag', index: 'flag', width: 40, align: 'center' },
            { name: 'supplierId', index: 'supplierId', width: 40, align: 'center' },
            { name: 'supplierFlag', index: 'supplierFlag', width: 40, align: 'center' },
            { name: 'templateCode', index: 'templateCode', width: 60, align: 'center' },
            { name: 'templateParam', index: 'templateParam', width: 120, align: 'center' },
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "templateId");
            }
        },
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    ...
                    var writeData = {
                        ...
                        cmd_col: "<input type='button' class='CommonButon' value='编辑' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='" + usetxt + "' onclick=\"button_actions('use" + rowData.isUse + "','" + id + "')\">"
                    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddMsgTemp.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        ...
        $.layer({
            ...
            title: ['详情', true],
            maxmin: true,
            iframe: { src: 'AddMsgTemp.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(flag, id) {
        ...
        layer.confirm('确定要' + context + '吗?', function () {
            var str = $.ajax({ url: "MsgTempHandler.ashx?flag=use&id=" + id + "&statu=" + state + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert(context + "成功！");
            }
            else {
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var id = "";
                for (var i = 0; i < rowData.length; i++) {
                    id += jQuery('#startech_table_jqgrid').jqGrid('getCell', rowData[i], 'templateId') + "|"; //获取所有选中的id值
                }
                //alert(id);
                var str = $.ajax({ url: "MsgTempHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
