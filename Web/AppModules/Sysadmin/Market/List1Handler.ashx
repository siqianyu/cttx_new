...
public class List1Handler : IHttpHandler
{
    ...
    {
        string table = "  IACenter_Market_User ";
        string fields = "uniqueid,username,truename,tel,mobile,isuse,addtime,'操作' as cmd_col";//字段顺序和必须前台jggrid设置的一样
    ...
    public string DeleteData(string ids)
    {
        ...
        if (ids.IndexOf(",") < 0)
        {
            res = adoHelper.ExecuteSqlNonQuery("delete from IACenter_Market_User where uniqueid='" + ids + "'");

        }
        else
        {
            ids = ids.TrimEnd(',');
            res = adoHelper.ExecuteSqlNonQuery("delete from IACenter_Market_User where uniqueid in('" + ids.Replace(",", "','") + "')");
