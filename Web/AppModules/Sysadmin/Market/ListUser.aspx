<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ListUser.aspx.cs" Inherits="AppModules_IACenter_ListUser" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <div class="Soso">
            <p class="Query">
                <span>用户名：</span><input style="width: 100px;" id="txtUsername" />
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "uniqueid");
            }
        },
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    var id = rowData.uniqueid;
                    if (rowData.isuse && rowData.isuse.toString() != "") {
                        var flag = { isuse: "" };
                        if (rowData.isuse.toString() === "1") {
                            flag.isuse = "<font style='color:green'>启用</font>";
                        }
                        else {
                            flag.isuse = "<font style='color:red'>禁用</font>";
                        }
                        $('#startech_table_jqgrid').jqGrid('setRowData', ids[i], flag);
                    }
    ...
    function button_actions(flag, id, lang) {
        if (flag == "add") {
            ...
            var str = $.ajax({ url: "List1Handler.ashx?flag=delete&lang='" + lang + "'id=" + id + "", async: false }).responseText;
        }
        else if (flag == "delete") {
            delete_method(id);
        }
    ...
    function grid_search() {
        var _StdID = $("#txtUsername").val();
        var _StdName = $("#txtstdname").val();
        var _searchfilter = "username$$" + $("#txtUsername").val() + "_$$_truename$$" + _StdName;
        var _searchfilter = escape(_searchfilter);


        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "List1Handler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            shade: [0.1, '#000'],
            fix: false,
            title: ['添加农贸市场管理员', true],
            maxmin: true,
            iframe: { src: 'AddUser.aspx?r=' + Math.random() },
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "List1Handler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
            jQuery("#startech_table_jqgrid").trigger("reloadGrid");
    ...
    function show_method(id) {
        $.layer({
            ...
            title: ['查看', true],
            maxmin: true,
            iframe: { src: 'AddUser.aspx?rd=1&id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function download_method(id, lang) {
        $.layer({
            ...
            title: ['查看', true],
            maxmin: true,
            iframe: { src: 'List1Handler.ashx?lang=' + lang + '&id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                for (var i = 0; i < rowData.length; i++) {
                    id += jQuery('#startech_table_jqgrid').jqGrid('getCell', rowData[i], 'uniqueid') + ","; //获取所有选中的id值
                }
                var str = $.ajax({ url: "List1Handler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
