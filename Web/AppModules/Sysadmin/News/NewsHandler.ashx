...
public class NewsHandler : IHttpHandler
{
    ...
    public string List(string curPage, string pageSize, string orderBy, string orderByRole, string searchFilter,string typeId)
    {
        ...
        string fields = "NewsId,title,CategoryName,ApprovedName,ViewCount,ReleaseDateName";//字段顺序和必须前台jggrid设置的一样
        ...
        if (orderBy.Equals("cmd_col")) { sort = " order by NewsId asc "; }
    ...
    public string GetFilter(string searchfilter,string typeId)
    {
        ...
        string filter = " 1=1 ";
        if (!typeId.Equals("0")) filter += " and CategoryId=" + typeId;
