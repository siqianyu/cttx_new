<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ZXList.aspx.cs" Inherits="AppModules_Sysadmin_Base_ZXList" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch"><div class="spanSearch" >咨询主题：</div><input style="width: 100px;" id="txtTitle" /></div>
                    <div class="divSearch"><div class="spanSearch" >回复状态：</div><asp:DropDownList ID="ddlApproved" runat="server" Width="102">
                            <asp:ListItem Value="">--请选择--</asp:ListItem>
                            <asp:ListItem Value="0">未回复</asp:ListItem>
                            <asp:ListItem Value="1">已回复</asp:ListItem>
                    </asp:DropDownList></div>
                    <div class="divSearch"><div class="spanSearch" >公开状态：</div><asp:DropDownList ID="dllPub" runat="server" Width="102">
                            <asp:ListItem Value="">--请选择--</asp:ListItem>
                            <asp:ListItem Value="0">未公开</asp:ListItem>
                            <asp:ListItem Value="1">已公开</asp:ListItem>
                    </asp:DropDownList></div>
                    <div class="divSearch"><div class="spanSearch" >咨询类别：</div><asp:DropDownList ID="ddlType" runat="server" Width="102">
                            <asp:ListItem Text="--请选择--" Value=""></asp:ListItem>
                            <asp:ListItem Text="产品相关" Value="0"></asp:ListItem>
                            <asp:ListItem Text="价格相关" Value="1"></asp:ListItem>
                ...
                <td style=" width:20%">
                    ...
                    <asp:Button ID="btnAdd" runat="server" Text=" 添 加 " Visible="false" OnClientClick="button_actions('add'); return false;" CssClass="Green InputClass"/>
                    <asp:Button ID="Button2" runat="server" Text=" 查 看 "  OnClientClick="button_actions('show');return false;" CssClass="Green InputClass"/>
                    <asp:Button ID="Button1" runat="server"  Text=" 审 核"  OnClientClick="return button_actions('approve')"  CssClass="Green InputClass" />
                    <asp:Button ID="BtnApprove" runat="server"  Text="取消审核"  OnClientClick="return button_actions('approveNo')"  CssClass="DarkRed InputClass" />
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'ZXHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['序号','咨询主题', '姓名', '恢复状态', '发布状态', '发布时间', '操作'],
        colModel: [
            { name: 'ID', index: 'ID', hidden: true },
            { name: 'title', index: 'title', width: 60, align: 'left' },
   		    { name: 'name', index: 'name', width: 40, align: 'left' },
            { name: 'stateName', index: 'state', width: 40, align: 'center' },
            { name: 'isCheckName', index: 'isCheck', width: 50, align: 'center' },
            { name: 'filltimeName', index: 'filltime', width: 60, align: 'center' },
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    var id = rowData.ID;
                    var writeData = { cmd_col: "<input type='button' class='CommonButon' value='回复' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='删除' onclick=\"button_actions('delete','" + id + "')\">" }
    ...
    function grid_search() {
        ...
        var _Pub = $("#dllPub").val();
        var _Type = $("#ddlType").val();
        var _searchfilter = "Title$$" + _Title + "_$$_Approved$$" + _Approved + "_$$_Pub$$" + _Pub + "_$$_Type$$" + _Type;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "ZXHandler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddZX.aspx?r=' + Math.random() },
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "ZXHandler.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
    ...
    var IsMore = 0;
        ...
        if (!IsCheck) {
            ...
            $.layer({
                ...
                title: ['查看', true],
                maxmin: true,
                iframe: { src: 'AddZX.aspx?rd=1&id=' + id },
                area: [document.body.scrollWidth - 20, $(document).height()],
                offset: ['0px', ''],
                close: function (index) {
                    //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function approve() {
        ...
        if (!IsCheck) {
            ...
            var str = $.ajax({ url: "ZXHandler.ashx?flag=approve&id=" + id + "", async: false }).responseText;
            jQuery("#startech_table_jqgrid").trigger("reloadGrid");
            if (str == "true") {
                alert("审核成功！");
            }
    ...
    function approveNo() {
        ...
        if (!IsCheck) {
            ...
            var str = $.ajax({ url: "ZXHandler.ashx?flag=approveno&id=" + id + "", async: false }).responseText;
            jQuery("#startech_table_jqgrid").trigger("reloadGrid");
            if (str == "true") {
                alert("取消审核成功！");
            }
            else {
                alert("取消审核失败，请于管理员联系！");
            }
        }
    }
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var str = $.ajax({ url: "ZXHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
