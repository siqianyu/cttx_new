<%@ Page Language="C#" AutoEventWireup="true" CodeFile="TSList.aspx.cs" Inherits="AppModules_Sysadmin_Base_TSList" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch"><div class="spanSearch" >内容：</div><input style="width: 100px;" id="txtTitle" /></div>
                ...
                <td style=" width:20%">
                  ...
                  <asp:Button ID="Button1" runat="server"  Text=" 审 核"  OnClientClick="return button_actions('approve')"  CssClass="Green InputClass" />
                    ...
                    <asp:Button ID="btnDelete" runat="server" Text="批量删除" OnClientClick="return button_actions('deleteAll')" CssClass="Red InputCla--%>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'TSHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['序号', '内容', '内容', '状态', '时间', '操作'],
        colModel: [
            { name: 'ID', index: 'ID', hidden: true },
            { name: 'Subject', index: 'Subject', width: 400, align: 'left' },
   		    { name: 'isOpenName', index: 'isOpen', width: 40, align: 'center', hidden: true },
            { name: 'IsPubName', index: 'IsPub', width: 40, align: 'center',hidden:true },
            { name: 'filltimeName', index: 'filltime', width: 50, align: 'center' },
    ...
    function grid_search() {
        ...
        var _searchfilter = "Subject$$" + _Title + "";
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "TSHandler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddTS.aspx?r=' + Math.random() },
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "TSHandler.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
    ...
    function show_method(id) {
        ...
        if (!IsCheck) {
            ...
            $.layer({
                ...
                title: ['查看', true],
                maxmin: true,
                iframe: { src: 'AddTS.aspx?rd=1&id=' + id },
                area: [document.body.scrollWidth - 20, $(document).height()],
                offset: ['0px', ''],
                close: function (index) {
                    //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function approve(YesOrNo) {
        ...
        if (!IsCheck) {
            ...
            var str = $.ajax({ url: "TSHandler.ashx?flag=" + YesOrNo + "&id=" + id + "", async: false }).responseText;
            jQuery("#startech_table_jqgrid").trigger("reloadGrid");
            if (str == "true") {
                alert("操作成功！");
            }
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var str = $.ajax({ url: "TSHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
