<%@ WebHandler Language="C#" Class="MemberType" %>
...
public class MemberType : IHttpHandler 
{
    ...
    {
        string table = "T_Base_MemberType";
        string fields = "TypeCode,TypeName,OrderBy";//字段顺序和必须前台jggrid设置的一样
        ...
        return JSONHelper.ToJGGridJson(curPage, totalPages.ToString(), totalRecords.ToString(), dtSource, fields.Split(','), "TypeCode");
    }
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        {
            if (hTable.Contains("TypeName") && hTable["TypeName"].ToString().Trim() != "") { filter += " and TypeName like '%" + hTable["TypeName"].ToString().Trim() + "%'"; }
            if (hTable.Contains("DisplayMode") && hTable["DisplayMode"].ToString().Trim() != "") { filter += " and DisplayMode=" + hTable["DisplayMode"].ToString().Trim() + ""; }
        }
        return filter;
    }
