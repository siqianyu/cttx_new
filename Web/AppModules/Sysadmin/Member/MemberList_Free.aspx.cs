...
public partial class MemberList_Free : StarTech.Adapter.StarTechPage
{
    ...
    {
        CreateExcel(DateTime.Now.ToShortDateString() + "免费会员列表");
    }
    #endregion
    ...
    {
        DataTable thisTable = DalBase.Util_GetList("select * from T_Member_Info where exists(select levelId from T_Member_Level where levelId=memberLevel and moneyType='Com' " + SetFilter() + ")").Tables[0];
        if (thisTable != null)
        {
            StringWriter sw = new StringWriter();
            sw.WriteLine("用户名\t公司名称\t剩余点数\t注册时间\t会员状态");
            foreach (DataRow dr in thisTable.Rows)
            {
                sw.WriteLine(dr["memberName"] + "\t" + dr["memberCompanyName"].ToString() + "\t"
                    + dr["freeMoenyAccount"].ToString() + "\t" + string.Format("{0:yyyy-MM-dd}", dr["regTime"]) + "\t"
