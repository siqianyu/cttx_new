<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ArticleList.aspx.cs" Inherits="AppModules_Sysadmin_Article_ArticleList" %>
        ...
        <table>
            <tr>
                <td style="width: 80%">
                    ...
                    <div class="divSearch"><span class="spanSearch">文章类型：</span><asp:DropDownList ID="ddlCategory" runat="server" Width="102">
                    ...
                    <div class="divSearch">
                        <span class="spanSearch">发布时间：</span>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'ArticleHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['序号', '文章标题', '类别', '审核', '发布时间', '操作'],
        colModel: [
            { name: 'ArticleId', index: 'ArticleId', hidden: true },
            { name: 'titie', index: 'titie', width: 100, align: 'left' },
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    ...
                    var title = rowData.titie;
                    var writeData = { titie: "<span onclick=\"button_actions('edit','" + id + "')\">" + title + "</span>", cmd_col: "<input type='button' class='CommonButon' value='编辑' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='删除' onclick=\"button_actions('delete','" + id + "')\">" }
    ...
    function grid_search() {
        ...
        var _Category = $("#ddlCategory").val();
        var _Approved = $("#ddlApproved").val();
        var _ReleaseDateBegin = $("#txtReleaseDateBegin").val();
        var _ReleaseDateEnd = $("#txtReleaseDateEnd").val();
        var _searchfilter = "Title$$" + _Title + "_$$_Category$$" + _Category + "_$$_Approved$$" + _Approved + "_$$_ReleaseDateBegin$$" + _ReleaseDateBegin + "_$$_ReleaseDateEnd$$" + _ReleaseDateEnd;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "ArticleHandler.ashx?flag=list&searchfilter=" + _searchfilter + "&typeId=" + GetQueryString("typeId"), page: 1 }).trigger("reloadGrid");
    }
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'AddArticle.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height() - 150],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "ArticleHandler.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
    ...
    function show_method(id) {
        ...
        if (!IsCheck) {
            ...
            $.layer({
                ...
                title: ['查看', true],
                maxmin: true,
                iframe: { src: 'AddArticle.aspx?rd=1&id=' + id },
                area: [document.body.scrollWidth - 20, $(document).height() - 150],
                offset: ['0px', ''],
                close: function (index) {
                    //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function approve(YesOrNo) {
        ...
        if (!IsCheck) {
            ...
            var str = $.ajax({ url: "ArticleHandler.ashx?flag=" + YesOrNo + "&id=" + id + "", async: false }).responseText;
            jQuery("#startech_table_jqgrid").trigger("reloadGrid");
            if (str == "true") {
                alert("操作成功！");
            }
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var str = $.ajax({ url: "ArticleHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
