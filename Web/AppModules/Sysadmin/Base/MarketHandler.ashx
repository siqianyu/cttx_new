<%@ WebHandler Language="C#" Class="MarketHandler" %>
...
public class MarketHandler : IHttpHandler
{
    ...
    public string List(string curPage, string pageSize, string orderBy, string orderByRole, string searchFilter)
    {
        ...
        string fields = "Market_id,Market_name,Area_Name,Map_x,Map_y,orderby";//字段顺序和必须前台jggrid设置的一样
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        if (hTable != null && hTable.Count > 0)
        {
            ...
            if (hTable.Contains("MarketId") && hTable["MarketId"].ToString().Trim() != "")
            {
                ...
                filter += " and Market_id like '%" + SafeStr + "%'";
            }
            if (hTable.Contains("MarketName") && hTable["MarketName"].ToString().Trim() != "")
            {
                ...
                filter += " and Market_name like '%" + SafeStr + "%'";
            }
    ...
    public string DeleteData(string ids, HttpContext context)
    {
        ...
        #region 默认农贸市场
        string mark = System.Web.Configuration.WebConfigurationManager.AppSettings["marketId"];
        foreach(string id in ids.Split('|'))
        {
            if (id == mark) { return "sys"; }
        }
        #endregion 
