<%@ Page Language="C#" AutoEventWireup="true" CodeFile="CouponList.aspx.cs" Inherits="AppModules_Sysadmin_Base_CouponList" %>
        ...
        <table>
            <tr>
                <td style=" width:80%">
                    ...
                    <div class="spanSearch" >优惠卷编号：
                    <input style="width: 120px;" id="txtCouponId" /></div>
                    </div>
                    
                                        <div class="divSearch">
                    <div class="spanSearch" >优惠卷描述：
                    <input style="width: 120px;" id="txtCouponContext" /></div>
                    </div>
                </td>
                <td style=" width:20%">
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'Coupon.ashx?flag=list',
        datatype: "json",
        colNames: ['优惠卷编号', '领取位置', '优惠类型', '面值', '描述', '开始时间', '结束时间', '二维码路径', '是否生效', '是否使用过', '备注', '操作'],
        //menuId,imgSrc,menuName,Technology,Flavor,CookingTime,Calorie,UserId,AddTime,isShow,orderby
        colModel: [
            { name: 'CouponId', index: 'menuId', width: 50, hidden: true },
            { name: 'GetPlaceInfo', index: 'GetPlaceInfo', width: 60, align: 'center' },
            { name: 'CouponType', index: 'CouponType', width: 50, align: 'center'},
            { name: 'CouponValue', index: 'menuName', width: 50, align: 'left' },
   		    { name: 'Context', index: 'Technology', width: 50, align: 'center' },
            { name: 'StartTime', index: 'Flavor', width: 90, align: 'center' },
            { name: 'EndTime', index: 'CookingTime', width: 90, align: 'center' },
            { name: 'QRCodeUrl', index: 'Calorie', width: 30, align: 'center', hidden: true },
            { name: 'isEffect', index: 'UserId', width: 50, align: 'center', hidden: true },
            { name: 'isUse', index: 'AddTime', width: 30, align: 'center', hidden: true },
            { name: 'Remark', index: 'isShow', width: 130, align: 'center' },
        ...
        onSelectRow: function (rowid) {
            if (rowid) {
                ...
                var id = $("#startech_table_jqgrid").getCell(gr, "CouponId");
            }
        },
    ...
    function grid_search() {
        ...
        var couponId = $("#txtCouponId").val();
        var couponName = $("#txtCouponContext").val();
        //var _Pid = $("#ddlPid").val();
        var _searchfilter = "CouponId$$" + couponId + "_$$_Context$$" + couponName; //+ "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "Coupon.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddCoupon.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'AddCoupon.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "Coupon.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else {
