<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MarketList.aspx.cs" Inherits="AppModules_Sysadmin_Base_MarketList" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch"><div class="spanSearch" >市场编号：</div><input style="width: 100px;" id="txtMarketId" /></div>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'MarketHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['市场编号', '市场名称', '所属区域', '经度','维度', '排序', '操作'],
        colModel: [
            { name: 'Market_id', index: 'Market_id', width: 50, align: 'center' },
            { name: 'Market_name', index: 'Market_name', width: 70, align: 'left' },
   		    { name: 'Area_Name', index: 'Area_Name', width: 70, align: 'center' },
            { name: 'Map_x', index: 'Map_x', width: 50, align: 'center' },
            { name: 'Map_y', index: 'Map_y', width: 50, align: 'center' },
    ...
    function grid_search() {
        ...
        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "MarketHandler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddMarket.aspx?r=' + Math.random() },
    ...
    function edit_method(id) {
        $.layer({
            ...
            title: ['修改', true],
            maxmin: true,
            iframe: { src: 'AddMarket.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "MarketHandler.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else if (str == "unusual") {
                ...
                alert("删除失败，不能删除系统默认农贸市场（请修改webconfig默认值）！");
            }
            else {
                alert("删除失败！");
            }
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var id = "";
                for (var i = 0; i < rowData.length; i++) {
                    id += jQuery('#startech_table_jqgrid').jqGrid('getCell', rowData[i], 'Market_id') + "|"; //获取所有选中的id值
                }
                var str = $.ajax({ url: "MarketHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
