<%@ Page Language="C#" AutoEventWireup="true" CodeFile="AreaList.aspx.cs" Inherits="AppModules_Sysadmin_Base_AreaList" %>
...
<body>
    ...
    <div class="SosoBar Left">
        <table>
            <tr>
                <td style=" width:80%">
                    <div class="divSearch"><div class="spanSearch" >区域编号：</div><input style="width: 100px;" id="txtAreaId" /></div>
                    <div class="divSearch"><div class="spanSearch" >区域名称：</div><input style="width: 100px;" id="txtAreaName" /></div>
                    <div class="divSearch"><div class="spanSearch" >父级区域：</div><asp:DropDownList ID="ddlPid" runat="server" Width="102"></asp:DropDownList></div>
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'AreaHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['区域编号', '区域名称', '父级区域', '所属层级', '排序', '是否为热门城市', '操作'],
        colModel: [
            { name: 'area_id',index: 'area_id', width:50, align: 'center' },
            { name: 'area_name', index: 'area_name', width: 70, align: 'left' },
   		    { name: 'PAreaName', index: 'PAreaName', width: 70, align: 'left' },
            { name: 'area_level', index: 'area_level', width: 50, align: 'center', hidden: true },
            { name: 'orderby', index: 'orderby', width: 30, align: 'center' },
            { name: 'ishot', index: 'ishot', width: 50, align: 'center' },
    ...
    function grid_search() {
        ...
        var _AreaName = $("#txtAreaName").val();
        var _Pid = $("#ddlPid").val();
        var _searchfilter = "AreaName$$" + _AreaName + "_$$_AreaId$$" + _AreaId + "_$$_Pid$$" + _Pid;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "AreaHandler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddArea.aspx?r=' + Math.random() },
    ...
    function delete_method(id) {
        layer.confirm('确定要删除吗?', function () {
            var str = $.ajax({ url: "AreaHandler.ashx?flag=delete&id=" + id + "&r=" + Math.random(), async: false }).responseText;
            if (str == "true") {
                alert("删除成功！");
            }
            else if (str = "unusual") {
                alert("删除失败，请先删除该区域下所有子级区域、小区、农贸市场！");
            }
            else {
                alert("删除失败！");
            }
    ...
    function deleteAction() {
        ...
        if (!IsCheck) {
            ...
            if (confirm('确定要批量删除这些吗?')) {
                ...
                var str = $.ajax({ url: "AreaHandler.ashx?flag=delete&id=" + id + "", async: false }).responseText;
                jQuery("#startech_table_jqgrid").trigger("reloadGrid");
                if (str == "true") {
                    alert("删除成功!");
                }
