<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Cost.aspx.cs" Inherits="AppModules_Sysadmin_Base_Cost" %>
                ...
                <td style="width: 80%">
                    <div class="divSearch">
                        <div class="spanSearch">
                            加工费名称：</div>
                        <input style="width: 100px;" id="txtMarketId" /></div>
                    <div class="divSearch">
                        <div class="spanSearch">
                            加工费备注：</div>
                        <input style="width: 100px;" id="txtMarketName" /></div>
                </td>
                <td style="width: 20%">
                    <div class="divSearchUC">
                        ...
                        <asp:Button ID="BtnApprove" runat="server" Visible="false" Text="批量审核" OnClientClick="return button_actions('approveAll')"
...
<script language="javascript" type="text/javascript">
    jQuery("#startech_table_jqgrid").jqGrid({
        url: 'CostHandler.ashx?flag=list',
        datatype: "json",
        colNames: ['序号','加工费名称', '加工费备注', '操作'],
        colModel: [
            { name: 'serviceId', index: 'serviceId', hidden: true },
            { name: 'serviceName', index: 'serviceName', width: 50, align: 'center' },
            { name: 'serviceContext', index: 'serviceContext', width: 70, align: 'left' },
        ...
        gridComplete: function () {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (rowData) {
                    ...
                    var writeData = { cmd_col: "<input type='button' class='CommonButon' value='编辑' onclick=\"button_actions('edit','" + id + "')\"> <input type='button' class='CommonButon' value='设置' onclick=\"button_actions('delete','" + id + "')\">" }
    ...
    function grid_search() {
        ...
        var _searchfilter = "serviceName$$" + _MarketId + "_$$_serviceContext$$" + _MarketName;
        var _searchfilter = escape(_searchfilter);

        jQuery("#startech_table_jqgrid").jqGrid('setGridParam', { url: "CostHandler.ashx?flag=list&searchfilter=" + _searchfilter + "", page: 1 }).trigger("reloadGrid");
    }
    ...
    function add_method() {
        $.layer({
            ...
            title: ['添加', true],
            maxmin: true,
            iframe: { src: 'AddCost.aspx?r=' + Math.random() },
    ...
    function delete_method(id) {
        $.layer({
            ...
            title: ['设置', true],
            maxmin: true,
            iframe: { src: 'AddCost1.aspx?id=' + id },
            area: [document.body.scrollWidth - 20, $(document).height()],
            offset: ['0px', ''],
            close: function (index) {
                //layer.msg('您获得了子窗口标记：' + layer.getChildFrame('#name', index).val(), 3, 1);
