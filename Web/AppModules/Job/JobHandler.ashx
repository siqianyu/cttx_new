...
public class JobHandler : IHttpHandler, IRequiresSessionState
{
    ...
    public string Disable(string goodsid, string statu)
    {
        ...
        string strSQL = "update T_Base_Job set if_leader='" + statu + "' where job_id='" + gid + "';";
    ...
    public string DeleteData(string ids, HttpContext context)
    {
        ...
        if (ids.IndexOf("|") < 0)
        {
            string strSQL = "delete from T_Base_Job where job_id='" + ids + "'";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        else
        {
            ids = ids.Replace("|", "','");
            string strSQL = "delete from T_Base_Job where job_id in('" + ids + "')";
            result = adoHelper.ExecuteSqlNonQuery(strSQL) > 0 ? "true" : "false";
        }
        return result;
    }
    ...
    {
        string table = " T_Base_Job";
        string fields = "job_id,job_name,if_leader, '操作' as cmd_col";//字段顺序和必须前台jggrid设置的一样
        ...
        if (statu != "")
        {
            ...
            if (statu == "1")
                filter += " and if_leader=1 ";
            if (statu == "0")
                filter += " and if_leader=0 ";
        }
        ...
        return JSONHelper.ToJGGridJson(curPage, totalPages.ToString(), totalRecords.ToString(), dtSource, fields.Replace("'操作' as ", "").Split(','), "job_id");
    }
    ...
    public string GetFilter(string searchfilter)
    {
        ...
        {
            string SafeStr = "";
            if (hTable.Contains("job_name") && hTable["job_name"].ToString().Trim() != "")
            {
                SafeStr = KillSqlIn.Form_ReplaceByString(hTable["job_name"].ToString().Trim(), Int32.MaxValue);
                filter += " and job_name like '%" + SafeStr + "%'";
            }
        }
        return filter;
    }
